...
const Comment = ({
  ...
  const { trigger: deleteTrigger } = useMutation(
    APIROUTE.MEETS_COMMENTS_EDIT(meetUpId, id),
    "DELETE"
  );
  ...
  const { trigger: ReplyTrigger } = useMutation(
    APIROUTE.MEETS_COMMENTS_REPLY(meetUpId, id),
    "POST"
  );
  const { trigger: LikeTrigger } = useMutation(
    APIROUTE.MEETS_COMMENTS_LIKE(meetUpId, id),
    "POST"
  );

  const onLikeClick = async () => {
    await LikeTrigger();
    mutate(APIROUTE.MEETS_DETAIL(meetUpId));
  };
  const onReplyClick = () => {
    setisEditting(false);
