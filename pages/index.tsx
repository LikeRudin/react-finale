  ...
  return (
    <Layout hasBottomBar hasTopBar title='모여서함께' seoTitle='모여서함께'>
      <div className='w-full max-w-xl max-h-screen overflow-y-auto bg-inherit flex flex-col justify-start    '>
        {!(
          meetUpList.status === "loading" || meetUpList.status === "error-init"
        ) ? (
          meetUpList.data.map((item, index) => {
            const {
              user: { username },
              id,
              name,
        ...
        {meetUpList.status === "loading" ||
        meetUpList.status === "error-init" ? null : (
          <button onClick={meetUpList.loadTrigger}>
            {meetUpList.status === "loading-more"
              ? "Loading..."
              : meetUpList.status === "reaching-end"
              ? "No more MeetUp"
              : "Load More"}
          </button>
        )}
